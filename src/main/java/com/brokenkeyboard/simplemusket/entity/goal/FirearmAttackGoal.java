...
public class FirearmAttackGoal extends Goal {
    ...
    private MusketState musketState = MusketState.UNLOADED;
    ...
    public void tick() {
        ...
        if (livingentity != null) {
            ...
            if (this.musketState == MusketState.UNLOADED) {
                if (!flag2) {
                    this.mob.startUsingItem(ProjectileUtil.getWeaponHoldingHand(this.mob, item -> item instanceof MusketItem));
                    this.musketState = MusketState.LOADING;
                    this.mob.setReloading(true);
                }
            } else if (this.musketState == MusketState.LOADING) {
                if (!this.mob.isUsingItem()) {
                    this.musketState = MusketState.UNLOADED;
                }
                ...
                if (stack.getItem() instanceof FirearmItem firearm && i >= firearm.getReload(stack)) {
                    ...
                    this.musketState = MusketState.LOADED;
                    this.attackDelay = 50 + this.mob.getRandom().nextInt(30);
                    this.mob.setReloading(false);
                    FirearmItem.setAmmoType(stack, 1);
                    FirearmItem.setReady(stack, true);
                }
            } else if (this.musketState == MusketState.LOADED) {
                --this.attackDelay;
                if (this.attackDelay == 0) {
                    this.musketState = MusketState.READY;
                }
            } else if (this.musketState == MusketState.READY && flag) {
    ...
    private boolean canRun() {
        return this.musketState == MusketState.UNLOADED;
    }
